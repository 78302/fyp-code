...
rnn = toy_vqapc(INPUT_SIZE=INPUT_SIZE, HIDDEN_SIZE=HIDDEN_SIZE, LAYERS=LAYERS).to(device)
...
if ORDER != 1:  # load the previous model
    path = './pretrain_model/model/Epoch{:d}_vq_{:s}.pth.tar'.format(ORDER-1, NAME)
    # print(path)
...
# Load loss:
try:
    losses = np.load(NAME + '_vq_losses.npy')
except:
...
#         min_valid_loss = valid_loss[-1]
    ...
    if ((i + 1) % 1 == 0):
        torch.save({'epoch': i + 1, 'state_dict': rnn.state_dict(), 'train_loss': train_loss,
                    ...
                    './pretrain_model/model/Epoch{:d}_vq_{:s}.pth.tar'.format(ORDER, NAME))
